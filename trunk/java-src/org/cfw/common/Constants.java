package org.cfw.common;

public class Constants {

    public static final String LOGIN_MODE    = "LoginMode";    // 登录模式

    public static final String ADMIN         = "admin";        // 超级管理员

    public static final String GUEST         = "guest";         // 匿名登录

    public static final String CURRENTUSER   = "currentUser";  // 当前登录用户信息

    public static final String CURRENTTHEME  = "currentTheme"; // 当前登录用户信息

    public static final String CURRENTMENUID = "currentMenuID"; // 当前菜单编码

    public static final int    GUEST_MASK    = 1;              // 功能菜单掩码为1时 允许guest用户访问
}
